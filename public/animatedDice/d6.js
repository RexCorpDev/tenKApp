...
D6.dice = function(numDice, callback, callbackData, useImages, buttonLabel) {
	...
	if (buttonLabel != "none") {
		genHtml += "<div id='diceform'><form><input type='button' ng-model='results' id='dicebutton' value='" + buttonLabel + "' onclick='D6AnimBuilder.get(\"dice\").reset(); D6AnimBuilder.get(\"dice\").start()' /></form></div>";
	}
	genHtml += "</div>";
	D6.genHtml = genHtml;
